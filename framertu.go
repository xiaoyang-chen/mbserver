...
func NewRTUFrame(packet []byte) (*RTUFrame, error) {
	...
	if len(packet) < 5 {
		return nil, errors.Errorf("RTU Frame error: packet less than 5 bytes 0x: % x", packet)
	}
	...
	if crcCalc != crcExpect {
		return nil, errors.Errorf("RTU Frame error: CRC (expected 0x: % x, got 0x: % x)", crcExpect, crcCalc)
	}
...
func (frame *RTUFrame) Addr() uint8 {
